...
/**
 * 买家管理（最终菜单 02）：所有注册买家。搜索/状态筛选 + 卡片表格（手机号脱敏、状态、
...
import { adminBuyers, type BuyerItem } from "@xmsn/api"

const message = useMessage()
const loading = ref(true)
const rows = ref<BuyerItem[]>([])
const keyword = ref("")
const status = ref<string | null>(null)

const drawerOpen = ref(false)
const current = ref<BuyerItem | null>(null)
...
const columns: DataTableColumns<BuyerItem> = [
  {
    title: "手机号",
    key: "phone",
    width: 150,
...
async function load(): Promise<void> {
  loading.value = true
  try {
    const res = await adminBuyers(keyword.value || null, status.value, 1, 100)
    rows.value = res.list ?? []
  } finally {
    loading.value = false
  }
...
<template>
  <div class="buyers-page">
    <div class="buyers-page__filters">
    ...
    <div class="buyers-page__table-card">
      <NDataTable
        :columns="columns"
        :data="filtered"
        :loading="loading"
    ...
    <NDrawer
      ...
      :title="`买家详情 ${current ? maskPhone(current.phone) : ''}`"
    >
      <NSpin :show="loading">
        <div v-if="current" class="buyers-page__drawer-body">
          <h4>基本信息</h4>
          <div class="buyers-page__kv">
            <div><span>手机号</span><b>{{ current.phone }}</b></div>
            <div><span>邮箱</span><b>{{ current.email ?? "—" }}</b></div>
            <div><span>角色</span><b>买家</b></div>
          ...
          <p class="buyers-page__hint">需求记录明细将在接入真实需求数据后展示。</p>
        </div>
      </NSpin>
    </NDrawer>
  </div>
...
<style scoped>
.buyers-page__filters {
  display: flex;
  gap: var(--space-12);
  margin-bottom: var(--space-16);
}
.buyers-page__table-card {
...
.buyers-page__kv div + div {
  border-top: 1px solid var(--color-border-subtle);
}
.buyers-page__kv span {
