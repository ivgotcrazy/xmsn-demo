...
| `buyer_requests` | 需求档案文本向量 | 1024 | request_id, conversation_id, version, user_id, product_type | 通道A需求侧 |
...
Response: { "request_id": "uuid", "version": 1, "redirect_to": "/buyer/matches/{request_id}" }
```
...
```mermaid
flowchart TD
    Input(["接收需求匹配快照<br/>（buyer_requests, version）"]) --> GenVector["生成需求向量<br/>（智谱 embedding-2）"]
