...
> - 《代理详细设计.md》—— 需求对话代理专项设计（整合与评估，含稳定性；本文档 3.4/6.4 引用）
> - 《匹配详细设计.md》—— 语义匹配引擎专项设计（双通道打分 LLD；本文档 6.2/6.3/6.5.3 引用）
> - 《厂商解析详细设计.md》—— 厂商能力解析（输入侧）专项设计 LLD（本文档 3.3/6.1/6.2 引用）
...
| ADR-11 | 契约先行 + 前端 mock 自测：M1 先定义全部接口层（Pydantic + router stub）产出完整契约，前端基于契约派生 MSW mock 先行开发 | 前后端真正并行；契约问题最早暴露 | mock 仅 dev，演示/生产禁用；SSE/上传/异步留真实联调 |
...
graph LR
    ...
    subgraph External["外部依赖"]
        ...
        EMB["智谱 Embedding API<br/>embedding-2（1024 维）"]
...
依赖要点：
- **LLM 与 Embedding 解耦**：DeepSeek 未提供 Embedding 端点，故 Embedding 用智谱 embedding-2（1024 维、中文强）；二者均可独立替换（本地演进备选 bge-m3，此时才需 GPU）。
...
graph TB
    subgraph Client["客户端"]
        UserWeb["客户端 / 厂商端<br/>user-web (Vue3)"]
...
| 域模块 | 职责 | AI 依赖 | 主要存储 |
| --- | --- | --- | --- |
| user-auth | 注册/登录/角色（vendor/customer/admin）/JWT | 无 | PG |
...
| 对象存储（PoC=本地盘） | 上传文档（PDF/PPT/Word） | 经 `FileStorage` 接口访问；PoC 用本地目录（演进 MinIO/OSS） |
...
│   │   │   ├── mocks/
│   │   │   │   └── handlers.ts      #   契约派生 MSW mock（仅 dev，见 5.5）
│   │   │   └── src/
...
│
├── infra/                           # 部署与运维（第 11 章）
│   ├── docker-compose.local.yml     # PoC 单机编排（11.2）
│   ├── docker-compose.remote.yml    # 远程部署端口收敛 override
│   ├── deploy_local.sh              # 本地一键部署（全铲重灌）
│   ├── deploy_remote.py             # 远程一键部署（本地构建→上传→load→运行）
...
| **Embedding** | 智谱 embedding-2 | 与 LLM 解耦、1024 维、中文强、国产 | 可换 bge-m3 本地（需 GPU） |
...
6. **存储**：文档存储本地盘（`FileStorage` 接口）→ 多实例/规模化时换 MinIO / S3 / OSS（与引入 Redis 同触发时机）。
...
- **鉴权**：JWT Bearer（`Authorization: Bearer <token>`），角色 vendor/customer/admin，管理员接口校验 admin；
...
### 5.5 前端 Mock 自测（契约派生 + 契约先行）
采用**契约先行**：M1 定义全部接口的 Pydantic schema + router 注册（handler 为 501 stub），`openapi.json` 以完整形状先行产出；前端据此派生 mock，可在后端实现前独立自测（ADR-11）。

```mermaid
graph LR
    CT["M1 契约层：Pydantic schema + router stub"] -->|"自动生成"| OA["openapi.json（形状完整）"]
    OA -->|"契约派生"| MK["前端 mocks/handlers.ts<br/>（MSW + faker）"]
    OA -->|"orval"| FE["src/client.ts<br/>类型+客户端"]
    MK -->|"dev 拦截"| APP["user-web / admin-web"]
```

**约定**：
1. **mock 由契约派生**：`generate.ts` 同时产出类型/客户端与 MSW handlers，字段/类型与后端一致；禁止手写固定 mock 造成第二真相源；
2. **仅 dev 生效**：`VITE_USE_MOCK=true` 时注入（默认 dev）；生产构建不含 MSW；**演示/生产不依赖任何 mock**（不违反“真实、无伪造”原则）；
3. **真实联调例外**：SSE 流式对话、文件上传、异步骨架轮询三类不做 mock 模拟，留真实后端联调（M3/M5/M7）；
4. **CI 兜底**：`openapi.json` 与契约基线 diff 校验，后端实现不得偏离契约；handler stub 统一返回 501。
...
CREATE TABLE vendor_capabilities (
  ...
  /*
  structured_tags 示例（key 与能力 Schema 一一对应，硬能力 7 项 + 软标签 3 项）：
  {
    "process_types": ["SMT贴片", "整机组装"],     // 硬
    "certifications": ["ISO9001", "CE"],          // 硬
    "os_support": ["Linux", "Android"],           // 硬
    "interfaces": ["网口", "USB", "HDMI"],        // 硬
    "moq": 3000,                                  // 硬（起订量）
    "lead_time_days": 20,                         // 硬（交期）
    "monthly_capacity": 500000,                   // 硬（月产能）
    "product_types": ["机顶盒", "智能音箱"],       // 软
    "application_scenarios": ["家庭娱乐", "智能家居"], // 软
    "customization": "支持ODM定制"                  // 软
  }
  */
  summary_text TEXT,  -- AI生成的一句话摘要（用于代表向量）
  version INT DEFAULT 1,     -- 档案版本号：每次重新解析（增/删文档）+1
  updated_at TIMESTAMP,      -- 档案生成时间
  doc_count INT DEFAULT 0,   -- 本次档案基于的文档数
  completeness FLOAT,      -- 能力完备度：硬能力已提取字段/硬能力总数
  source_map JSONB,        -- 字段级溯源+置信度 {field_key: {doc_id, doc_name, page, chunk_text, confidence}}
  source_type VARCHAR(50) DEFAULT 'doc',  -- 仅文档录入，恒为 doc
...
CREATE TABLE match_results (
  ...
  request_id UUID REFERENCES customer_requests(request_id),
  vendor_id UUID REFERENCES vendors(vendor_id),
...
| --- | --- | --- | --- | --- |
| `vendor_representative` | 厂商级代表向量（structured_tags拼装+summary_text，1档案=1向量） | 1024（智谱 embedding-2） | vendor_id, capability_id | 通道A厂商Top-K检索 |
| `doc_chunks` | 厂商原始文档按段落/页切块的向量 | 1024 | vendor_id, doc_id, doc_name, page, chunk_text | 引用溯源（“查看原文”） |
| `customer_requests` | 需求档案文本向量 | 1024 | request_id, conversation_id, version, user_id, product_type | 通道A需求侧 |
| `knowledge_base` | 领域知识文本向量 | 1024 | knowledge_id, category, industry | 对话Agent RAG |
...
**POST /api/v1/conversation/start** —— 开始新的对话（客户端）
```json
...
**POST /api/v1/conversation/confirm** —— 确认并提交匹配（**单一端点**，无独立 finish 步骤）→ 生成需求匹配快照（version++）+ 触发匹配
```json
Request: { "conversation_id": "uuid", "demand_points": [] }
Response: { "request_id": "uuid", "version": 1, "redirect_to": "/customer/matches/{request_id}" }
```

#### 6.3.2 厂商能力相关接口

**POST /api/v1/vendor/capability/upload** —— 上传能力文档（仅文档，AI 解析）
```json
Request: Multipart/form-data
{ "vendor_id": "uuid", "documents": [...] }   // 无 form_data / free_text
Response: {
  "capability_id": "uuid",
  "structured_tags": {...},                   // 与能力 Schema 一一对应
  "summary_text": "...",                      // 正式档案（只读，无预览/编辑）
  "version": 3,                               // 档案版本（每次增删文档重新解析+1）
  "updated_at": "2026-08-10T08:00:00Z",      // 档案生成时间
  "doc_count": 2,                             // 基于的文档数
  "completeness": 0.857,                      // 能力完备度（硬能力覆盖比例）
  "source_map": { "process_types": {"doc_name": "...", "page": 1, "confidence": 0.92} },  // 字段级溯源+置信度
  "doc_urls": ["..."],
...
#### 6.3.5 历史与文档接口（客户端，对应产品 2.8）

**GET /api/v1/conversations** —— 客户会话列表
```json
Response: {
  "conversations": [
...
### 6.4 Prompt 设计（职责与归属）
Prompt 全文属于各详细设计的实现层；本文档只定职责与归属，各 Prompt 的全文、模板、Few-shot 与调用方式见对应详细设计。

| Prompt | 用途 | 宿主详细设计 |
| --- | --- | --- |
| 对话 System Prompt | 需求萃取（角色/Schema/画像/知识/历史/槽位 7 段装配） | 《代理详细设计》3.1~3.2 |
| 厂商解析 Prompt | 资料 → 能力档案提取 | 《厂商解析详细设计》 |
| 匹配理由 Prompt | verdict 三组 + AI 评语 + source | 《匹配详细设计》8.5 |
| 通道B 判定 Prompt | 语义参数 verdict 批量判定 | 《匹配详细设计》4.3 |
| 画像提取 Prompt | 画像增量建议（updates） | 《用户画像设计》4.2 |
...
sequenceDiagram
    ...
    User->>FE: 确认需求
    FE->>Agent: 提交最终结构化需求
    Agent->>DB: 存储会话 conversations + 需求匹配快照 customer_requests（version递增）
    Agent->>VDB: 生成需求向量并存储
...
sequenceDiagram
    ...
    V->>FE: 上传能力文档（PDF/PPT/Word）
    FE->>API: POST /vendor/capability/upload（documents[]）
    API->>Parse: 提交解析（仅文档）
    Parse->>Parse: 提取文本（Unstructured/PyPDF2）
    Parse->>Parse: LLM生成 structured_tags + summary_text<br/>+ source_map（含字段级 confidence）
    Parse->>Parse: 计算 completeness（硬能力覆盖比例）
    Parse->>VDB: ①代表向量入库 vendor_representative
    Parse->>VDB: ②原文块向量入库 doc_chunks
    Parse->>PG: 写入 vendor_capabilities（audit_status=pending）
    Parse-->>API: 返回能力档案（只读）
    API-->>FE: 返回档案 + completeness/source_map（字段级置信度）
    FE-->>V: 提示“资料已提交，进入审核中”
...
```mermaid
flowchart TD
    Input(["接收需求匹配快照<br/>（customer_requests, version）"]) --> GenVector["生成需求向量<br/>（智谱 embedding-2）"]
...
### 6.6 需求 Schema 品类定义
> 需求 Schema = **固定字段（共通维度）** + **品类扩展（按 `product_type` 加载）** + **开放扩展（`extra_constraints`）**；所有字段支持三态（已指定 / 未指定通配 / 明确排除）。

#### 6.6.1 固定字段（共通维度，参与通道B计分）
| key | 类型 | 说明 |
| --- | --- | --- |
| `product_type` | string | **唯一必填品类锚点**：机顶盒 / 智能音箱 / IoT设备 / 其他 |
...
#### 6.6.2 品类扩展（按 `product_type` 加载）
| 品类 | key | 类型 | 枚举 / 说明 |
| --- | --- | --- | --- |
| **机顶盒** | `decode_capability` | string[] | H.264 / H.265 / AV1 / HDR |
| | `soc_platform` | string | Amlogic / Allwinner / HiSilicon / Rockchip |
| | `wireless` | string[] | WiFi / 蓝牙 / 无 |
| | `tv_standard` | string[] | DVB-C / DVB-T / ATSC / ISDB / 无 |
| | `output_interfaces` | string[] | HDMI / AV / SPDIF |
| | `memory_storage` | string | 如 2G+16G |
| **智能音箱** | `mic_array` | string | 2麦 / 4麦 / 6麦 |
| | `speaker_power` | string | 3W / 5W / 10W |
| | `voice_assistant` | string | 自研 / 接入第三方 |
| | `wireless` | string[] | WiFi / 蓝牙 |
| **IoT设备** | `comm_protocol` | string[] | BLE / WiFi / Zigbee / LoRa / NB-IoT / 4G |
| | `power_supply` | string | 电池 / 市电 / USB |
| | `ip_rating` | string | IP54 / IP65 / 无 |
| | `sensors` | string[] | 温度 / 湿度 / 运动 / 无 |
| **其他** | — | — | 无预置扩展，走开放扩展（`extra_constraints`） |

#### 6.6.3 使用与衔接
- **Agent**（《代理详细设计》）：追问顺序、选项按钮、完成判定"关键维度"均来自当前品类扩展（如机顶盒 → 解码/主控/无线等）；`product_type` 未确认前不进入匹配；
- **匹配**（《匹配详细设计》）：确认后，品类扩展字段加入 `PARAM_MAP`（默认权重 1.0，可配置），参与通道B计分；三态同样生效；
- **扩展字段三态**：已指定 / 未指定（通配，不计入分母）/ 明确排除（负向过滤）；
- **动态品类 Schema 生成**（按用户自由输入自动建模）列入 Won't / 演进项（核心需求）。
...
| 上传文档原件 | 对象存储（PoC=本地盘） | URL 存 PG（doc_urls / license_url）；经 `FileStorage` 接口，演进换 MinIO/OSS |

### 7.2 关键数据流（端到端）
1. **供给侧**：厂商资料 → 解析（异步 Worker）→ 能力档案落 PG → 代表向量 + 原文块向量入库 Milvus → 审核 passed 后进入匹配候选池；
2. **需求侧**：多轮对话（RAG + 画像注入）→ 需求档案（版本化）→ 确认 → 需求匹配快照（`customer_requests.version` 递增）→ 触发匹配；
...
### 8.3 GPU 决策
- **PoC 不需要 GPU**：推理走托管 LLM API（DeepSeek），Embedding 走 API（智谱 embedding-2），服务器无自建推理负载；4 核 8G 云服务器即可演示；
...
| 层面 | 措施 |
| --- | --- |
| **认证授权** | JWT Bearer；角色 vendor/customer/admin；管理员接口校验 admin；口令 bcrypt/argon2 哈希 |
...
| 维度 | 设计 |
| --- | --- |
| **日志** | 结构化 JSON 日志（stdout）；**指标类**，可聚合/可清理（区别于事实数据） |
| **事实数据** | Agent 会话与 LLM 输入输出以 **append-only** 保存（`llm_call_logs`/`conversation_events`，见《代理详细设计》3.3）：**只增不删不改、输入全量、长期保留**，供追溯/评估/重放/优化 |
...
graph TB
    subgraph Compose["docker-compose 单机"]
        ...
        OBJ["/data/uploads 卷<br/>FileStorage 本地实现"]
    end
    NGINX --> WEB
    NGINX --> API
    API --> PG
...
- **备份**：PostgreSQL 定时备份；Milvus 数据随源文档可重建（上传文档保留于本地盘，经 `FileStorage` 读取）；
...
| 阶段 | 任务 | 预计工期 | 依赖 |
| --- | --- | --- | --- |
| **M1** | 工程骨架 + 契约先行：monorepo + compose + PG/Milvus + 全部接口契约 + mock 链路 | 4天 | 无 |
...
| 需求匹配快照 | `customer_requests` 中按 version 存储的确认时需求 |
