...
> 版本：v1.1 ｜ 状态：指导编码（已对齐 AI核心 Stage0-4） ｜ 更新：2026-08-16
>
> 本文档是**语义匹配引擎**的详细设计（Low-Level Design），目标**可直接指导编码**：包含类型定义、判定算法伪代码、字段映射表、流程图、组件接口、兜底路径与关键用例。
>
> **相关文档**：《AI核心总体架构设计v2.md》（Stage0-4 漏斗权威描述）、《系统架构设计.md》（6.2 向量库 / 6.3.3 匹配接口）、《核心系统需求.md》（3.2 语义匹配引擎）、《代理详细设计v2.md》（Agent 产出需求档案为匹配输入）、《系统开发计划.md》（M4 匹配引擎）。
>
> **⚠ 设计变更（2026-08-16，对齐 AI核心 D5-D11 + Stage0-4，未全文重写）**：本文件原为**双通道（通道A/B）** LLD；顶层流程已重构为 **Stage0-4 漏斗**，本文件保留 LLD 细节（类型/算法/兜底/用例），章节↔Stage 映射如下（权威描述见 AI核心 §5.3）：

| 旧（双通道） | 新（Stage0-4） |
| --- | --- |
| 无 | **Stage0 硬过滤**：SQL 零 LLM，strict 维度（闭集/无容差）→ `passed` 集（AI核心 §5.3.1） |
| 通道A 语义检索（§3） | **Stage1 混合召回**：passed 内 两路 ANN（代表 ∪ 原文块）；semantic 只做召回不进分（D9/D10，§5.3.2） |
| 通道B 参数判定（§4） | **Stage2 逐维度判定**：value_type 派生 + strictness 接受；verdict 四档（matched/partial/missing/unmatched，§5.3.3） |
| 综合打分（§5） | **Stage3 打分**：需求点等权平均 `round(Σ档位/N)`，阈值 60（D10，§5.3.4） |
| 解释生成（§8） | **Stage4 LLM 解释**：仅 Stage3 的 TopK，四组 + match_reason + risk_warning（§5.3.5） |
...
class MatchResultRow(TypedDict):
    ...
    capability_id: str
    match_score: float
    semantic_score: float            # 仅召回用（D10 不进最终分）
    match_source: MatchSource
    judgements: list[ParamJudgement]  # 落库前按 verdict 拆为四组
```
...
class Scorer(Protocol):
    """等权需求点打分 + 阈值过滤（D10）"""
    def score(self, judgements: list[ParamJudgement]) -> MatchScore
...
```python
CONFIG = {
  "channel_a": {"top_k": 50, "min_semantic": 0.35},   # 召回参数（5.3.6 ② 待确认）
  "verdict_score": {"matched": 100, "partial": 50, "missing": 30, "unmatched": 0},  # D10 需求点四档等权
  "threshold": {"min_match_score": 60.0},
  "explain": {"top_n": 10},  # 解释范围 = Stage3 输出的 TopK（K=10，④ 定案）
}
```
...
| 2 | **分母=已指定需求点数** | 只对已指定需求点计分，未指定不计入，少填不稀释 |
| 3 | **需求点等权（D10）** | 无权重；strict 维度由 Stage0/2 门槛兜住，best-effort 等权计分；语义维度取 verdict 三档 |
...
async def compute(request_id: str) -> ComputeResult:
    ...
    for c in cands:
        ...
        sc = scorer.score(jud)                        # 等权打分 + 阈值（5.2，D10）
...
def demand_embedding_text(demand: dict) -> str:
    """D9：自然语言需求描述（程序模板），与厂商 summary 同构，保证向量空间对称。"""
    dims = demand.get("dimensions", {})
    parts = []
    if pt := (dims.get("product_type") or {}).get("value"):
        parts.append(f"需要{pt}")
    if os_ := (dims.get("os") or {}).get("value"):
        parts.append(f"支持{'、'.join(os_)}系统")
    if cert := (dims.get("certifications") or {}).get("value"):
        parts.append(f"认证{'、'.join(cert)}")
    if moq := (dims.get("moq") or {}).get("value"):
        parts.append(f"起订量{moq}")
    if lead := (dims.get("lead_time_days") or {}).get("value"):
        parts.append(f"交期{lead}天内")
    if scen := (dims.get("application_scenario") or {}).get("value"):
        parts.append(f"场景{scen}")
    for c in demand.get("extended", []):
        parts.append(f"{c.get('label','扩展')}:{c.get('value')}")
    return "；".join(parts)
# 示例："需要智能音箱；支持Linux、Android系统；认证CCC、SRRC；交期30天内；场景家庭客厅；外观:外壳黑色；物流:送货上门"
```
...
```python
PARAM_MAP = [
  # (规范参数, 权重, 需求字段, 厂商能力字段, 判定方式)  # ⚠ 遗留表：D1 将改为本体同 key 派生、D10 取消权重，待匹配漏斗改造重写
...
```markdown
给定客户需求 {demand_json} 与厂商能力 {capability_json}，
...
### 4.4 汇总（match_score = 需求点命中率，D10 等权）
```python
def match_score(judgements: list[ParamJudgement]) -> float:
    """分母=已指定需求点数（未指定不计入）；返回 0-100 取整"""
    if not judgements:
        return 100.0
    return floor(sum(VERDICT_SCORE[j["verdict"]] for j in judgements) / len(judgements))

VERDICT_SCORE = {MATCHED: 100, PARTIAL: 50, MISSING: 30, UNMATCHED: 0}   # D10 四档等权
```
...
### 5.1 公式（D10 等权）
$$\text{match\_score} = \left\lfloor \frac{\sum \text{需求点匹配度}}{\text{需求点数}} \right\rfloor$$

> 需求点匹配度四档：matched=100 / partial=50 / missing=30 / unmatched=0（missing=未声明，介于 partial 与 unmatched 之间）。语义维度取 LLM verdict 档位，与规则点同量纲。
...
def score(judgements: list[ParamJudgement]) -> MatchScore:
    return {"match_score": match_score(judgements)}      # 4.4（0-100 等权）
```

### 5.3 过滤规则
- `match_score < 60` → 剔除（不进结果）；
- 排序：`match_score` 降序 → 取 TopK（默认 10，可配）；**Stage4 解释范围 = 该 TopK**（④ 定案）。
- strict 未满足（strict_ok=false）的厂商由 Stage0/2 门槛处理，不在此另设封顶（D10）。
...
def to_row(cand, sc, judgements) -> MatchResultRow:
    return {
        ...
        "semantic_score": cand["semantic_score"],   # 仅召回用（D10 不进最终分）
        "match_source": source,
        # 四组 JSONB（与 verdict 四档一一对应；missing 独立成组，不并入 partial）
        "matched_params":   [j for j in judgements if j["verdict"] == MATCHED],
        "partial_params":   [j for j in judgements if j["verdict"] == PARTIAL],
        "missing_params":   [j for j in judgements if j["verdict"] == MISSING],
        "unmatched_params": [j for j in judgements if j["verdict"] == UNMATCHED],
    }
```

### 7.2 查询接口
- `GET /match/detail/{id}`：返回行 + 四组判定 + `ai_comment`（异步生成，未生成返回骨架标记）；
...
### 8.1 输入：Agent 快照
匹配只消费 `customer_requests` 快照（Agent 确认时生成，见《代理详细设计》8.1 `on_confirm`）；**不访问对话历史**，保证输入纯净。
...
Response: {
  "match_results": [ {vendor_id, company_name, location, summary,
                      ...
                      match_source, matched_count, partial_count, missing_count, unmatched_count} ],
  "total_matches": int,
...
- 解释结果（四组判定 + `ai_comment` + `source`）回写 `match_results`，详情接口按需读取；
...
```markdown
你是一个供需匹配分析专家。给定客户的需求和一个厂商的能力，分析匹配情况并生成解释。
...
| ID | 用例 | 期望结果 |
| --- | --- | --- |
| UC-M1 | 同款产品+同OS+同认证 | 全需求点 matched → `match_score=100` |
| UC-M2 | 交期 30 vs 45 | `lead_time_days` = partial（可协商） |
| UC-M3 | 需求要 CE 而厂商无 | `certifications` = unmatched（strict）→ Stage0 淘汰；best-effort 则计 0 分不硬杀（D10） |
| UC-M4 | 需求排除 HDMI，厂商支持 | **硬过滤淘汰**，不进结果 |
| UC-M5 | 未指定 OS | `os_support` 不计入分母（不稀释分数） |
| UC-M6 | `match_score < 60` | 剔除，不进结果 |
