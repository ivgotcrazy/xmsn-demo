...
graph LR
    ...
    PC --> RESP
    CONF -->|"生成快照+触发匹配"| SNAP["customer_requests(version++) + match_runs"]
    SNAP --> RESP
```
...
- **确认路径（DONE/CONFIRM）**：`confirm_submit` 生成需求快照（`customer_requests` version++）+ 触发 `match_runs` → `SNAP` 落库（含物化统计）→ `respond` 返回跳转；会话置 closed（8.1）。
...
async def on_confirm(state: AgentState) -> MatchTrigger:
    ...
    # 1) 落库 customer_requests（快照）  2) 生成需求向量  3) 触发 POST /match/compute
...
```mermaid
graph LR
    AGENT["Agent：需求档案（确认）"] -->|"快照 customer_requests(version)"| MATCH["双通道匹配"]
    MATCH --> RES["match_results"]
    RES -->|"命中率反馈"| EVAL["评估体系（7）"]
    AGENT -->|"需求侧行为驱动"| PROFILE["用户画像"]
    PROFILE -.->|"新会话注入引导"| AGENT
```
- **画像只由需求侧行为驱动**（确认的 `structured_demand` + 对话显式偏好，异步提取）；**`match_results` 不进入画像**——① 语义错位（适配度 ≠ 长期偏好）② 数据隔离（厂商数据不混入客户画像，核心需求 3.4.2）③ 避免"结果→画像→引导→需求→结果"反馈环污染稳定性与评估纯净（7.2）；
