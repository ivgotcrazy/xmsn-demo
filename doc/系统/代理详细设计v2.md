...
- 服务层 `domains/conversation/service.py`：
  ...
  - `confirm()`：锚点缺失报 400 → 生成 `BuyerRequest`（`version++`、`structured_demand=_slots_snapshot`）→ 建 `MatchRun(running)` → **`conv.status="confirmed"`** → 异步画像更新。
- 意图/合并逻辑 `domains/conversation/agent.py`：
  - `route_intent()`：`option_click / confirm / done / help / free_text`（关键词规则，**无第 10 章 B/C 档正则**——实际代码未含 `_REDIRECT_PAT/_OFFTOPIC_PAT`）。
...
- 需求级：`current_slots` 三态槽位 + `extra_constraints`；快照 `_slots_snapshot`（三态）落 `BuyerRequest.structured_demand`。
...
| 现状 | 目标 | 改动落点 |
| --- | --- | --- |
| `route_intent` 四意图（option/confirm/done/help/free_text） | **意图路由（②节点）**：仅强指令拦截 + 纯引导模板；自由文本一律进推理节点；补 B/C 档正则 | `agent.py` route_intent 扩展 |
...
| `completion_ready` + KEY_DIMS | **动态 Validator**：按品类 `fields_for(pt)` 分级覆盖（9.3） | `schema.py` + `agent.py` |
| `decide_question` 按序追问 | **`pending_slots(pt,state)`**：级别 + 依赖排序，注入 `next_slot/allowed_set`（9.3） | `schema.py` 新增 + 推理节点注入 |
| `conv.status=confirmed/closed` | 会话级只留 `active/删除`；提交下沉 `BuyerRequest`/`MatchRun`（第 10 章） | `service.py` confirm/message |
...
- **范围**：
  ...
  2. **提交入口分级**（SC-22/22b/25）：直接提交（选项/强命令/`/done`）+ 弱收尾复述引导（RECAP）+ 未达判定强命令提交警示。
  3. **完成判定**：字段分级（hard/soft/optional）+ 动态 Validator + **reconcile** + **pending_slots 注入**（第 9 章）。
  4. **死循环/失败熔断**：`_stall_counter`。
  5. 扩展/槽位回显（已部分完成）。
  6. **会话状态重新设计**（第 10 章，评审确认纳入 L1）：`confirm()` 不再写 `conv.status="confirmed"`；`Conversation.status` 只留 active/删除；前端"已提交"提示改派生值（`buyer_requests` 计数 / 最近匹配时间）。
...
- **验收**：萃取答疑（SC-17~20）、引导（SC-26~35）走通；同文案重复 = 0；`pending_slots`/`next_slot` 落 `conversation_events`；回归现有对话链路（容器内 e2e）。
...
> **提交入口策略**：确认提交支持"聊天内提交"，按意图明确度**分级触发**，统一收敛到单端点 `/conversation/confirm`：
> - **直接提交（明确授权）**：UI 按钮 / 聊天选项"确认完成" / 强命令 `/done` / 精确短语"确认并提交匹配" → 快照 vN + 触发匹配 → 结果页；
> - **引导确认（弱收尾）**："就这样吧/差不多了/可以了" → 摘要档案 + 高亮缺项 + 亮出确认按钮（**不直接提交**）。

| SC | 场景 / 输入 | 处理策略 | 流转落点 | 支持度 |
| --- | --- | --- | --- | --- |
| SC-21 | 完成判定达成 | 主动确认提示（仅一次） | 确认提示 | ✅已修 |
| SC-22 | 明确授权提交（强命令/选项） | 快照 vN + 触发匹配 → 结果页 | 结束 | ✅ |
...
| SC-32 | 帮助/能力边界 | 能力说明 + 拉回 | 帮助→回萃取 | L1 |
...
5. **策略由规则定、表达由模型做**：问什么/引导/答疑由规则层决定，模型只负责理解与表达——可控、可评估、可回归。LLM 是优秀的"文本处理器/语义路由器"，但它是糟糕的"状态机"；状态流转交给确定性代码，模型只做 NLU/NLG。
...
```
策略层 policy
  规则路由：强指令拦截 / 纯引导模板 / 自由文本→推理节点 / 完成判定 / 熔断
      │
      ...
      └─ skill: confirm         (SC-22/22b/25：快照 vN + 匹配)
统一响应: reply_text + slot_delta + extra + intent + options（一次 LLM 调用：既解析又说话）
```

> **实现映射**：L1 将流转图实现为"路由 + 函数化 Handler"（每 skill 一个函数，`route_intent` 确定性分发），不引入职责链/编排框架；L3 若状态机分支复杂再 DAG 化（LangGraph），handler 签名保持不变。
...
- **轮次判定（意图由推理节点产出，评审确认 2026-08-13 路径A）**：`intent=extract`（有 `tool_call`）→ 解析 `slot_delta/extra` → `merge_slot` → 状态修剪 → 完成判定 → 追问；`intent=qa` → 领域答疑（知识 RAG，短答+引导回槽位）；`intent=guide_result/guide_back` → 引导模板（看详情/回萃取）；`intent=weak_close` → 复述+引导确认（不提交）；`intent=recommend` → 建议值+确认（SC-05）。
- **混合意图一次处理**：自由文本（含"问结果 + 填槽"混合）统一进推理节点，LLM 同时产出 `reply_text`（可含引导）与 `tool_call`（槽位更新），不再由规则硬分流；`intent` 为**结构化输出**供策略层校验/审计。
- **意图的机械实现（双工具）**：`update_requirement_slots`（填槽）+ `classify_turn{intent, reply_text}`（非填槽轮：答疑/引导/弱收尾/推荐）；`tool_choice="auto"`，模型按上下文选择；有 `tool_call` → extract，否则 `classify_turn` 给出 intent+正文。混合意图由 `update_requirement_slots` + content 双通道一次处理。
- **校验分离**：`tool_call` 参数必须过 Pydantic + 枚举；`reply_text` 仅限长 + 去 markdown。
...
graph TD
    ...
    RT -->|"采纳/拒绝/跳过建议（_recommend）"| WRITE
    RT -->|"自由文本（含混合意图）"| RX["④ 推理节点 Agent Reasoning 🤖<br/>完整上下文+ RAG → 一次推断：<br/>intent(结构化) + reply_text + tool_call(slot_delta/extra)<br/>intent: extract/qa/guide_result/guide_back/weak_close/recommend"]
...
| --- | --- | --- | --- | --- |
| ① 消息入口 | 🧩 否 | 接收文本/选项点击 | 全部 | — |
| ② 意图路由 | 🧩 否 | 仅拦**无歧义命令**（选项点击/强命令/帮助 + 采纳·拒绝·跳过建议[状态条件]）；自由文本一律进推理节点 | 全部 | 语义判定下沉推理节点；不按内容正则分流 |
| ③ 写槽位 | 🧩 否 | 选项确定性直写 | SC-06/13 | 不调 LLM |
| ④ 推理节点 | 🤖 **是** | 完整上下文一次推断：**intent（extract/qa/guide_result/guide_back/weak_close/recommend）+ reply_text + tool_call(slot_delta/extra)**；内含 extract_slots / domain_qa / recommend_slots | SC-05~20/28~35 | 混合意图一次处理；Tool Calling；意图=上下文判定 |
...
| ⑧ 确认提交 | 🧩 否 | 快照 vN + 触发匹配（提交事件，会话保持 active） | SC-22/25 | 单端点 |
| ⑨ 应答 | 🧩 否 | reply_text + options + 档案对齐 | 全部 | 回入口（循环） |
| 熔断 | 🧩 否 | 连续失败/无进展 → 换方式/引导人工 | SC-37/38/33 | `_stall_counter` |

> **LLM 使用点汇总**：全图仅 2 处调 LLM——**④ 推理节点**（每轮自由文本的主处理，一次推断产出回复+槽位+意图）与 **recommend_slots**（SC-05 建议值生成，可复用 ④ 的产出）。其余 12 个节点全部是确定性/纯代码（模板、规则、状态机、装配），零 LLM 成本、可单测、可回归。这也是"把 LLM 当糟糕的状态机、当优秀的语义处理器"的落地：**凡是"状态变更/判定/引导"都是代码，凡是"理解/表达"才是模型**。
...
> - **确定性层（`route_intent`，无歧义才拦）**：选项点击 / 强命令（`/done`、确认并提交）/ 帮助 / 采纳·拒绝·跳过建议（仅 `_recommend` 存在时，状态条件）。**不再按关键词拦截** 弱收尾/引导/推荐/答疑。
> - **语义判定下沉推理节点**：弱收尾(SC-22b)/引导(B·C)/推荐(SC-05)/答疑 由推理节点在**完整上下文**（当前槽位 + next_slot + 历史 + 画像 + 知识）判定并输出结构化 `intent`；话术模板（复述/引导/建议）仍为确定性代码。
> - **B/C 合规安全覆盖（政策层后置保险）**：对"厂商评价唯一权威=匹配结果"（红线3），政策层用强正则做**后置覆盖**——模型产出非引导意图但原文强命中厂商/结果模式时，正文强制替换为引导模板（不参与主路由，只兜底合规）。
> - **模式配置化**：B/C 安全覆盖的模式表做成配置数据，加模式不改代码。
...
#   Intent = OPTION_CLICK | CONFIRM_COMMAND | HELP | ACCEPT/DECLINE/SKIP_RECOMMEND | FREE_TEXT（仅无歧义命令）
async def guide_override_for(text: str) -> str | None: ...                  # B/C 合规后置保险（不参与主路由）
...
async def confirm_submit(db, conv, user_id, mode) -> ConfirmResult: ...     # ⑧ 快照+匹配

# 技能层（推理节点内，一次 LLM 调用；意图=上下文判定）
async def agent_reasoning(state, message, ctx) -> ReasoningOutput: ...      # ④ 一次推断
#   ReasoningOutput = { intent, reply_text, tool_call(slot_delta/extra) | None }
#   intent ∈ {extract, qa, guide_result, guide_back, weak_close, recommend, empty}
...
> Schema 非扁平键值对，而是**带级别与依赖关系**的树（选 A 才追问 B）。本章定义数据结构扩展与算法，是动态 Validator（⑥）与动态待填集注入（追问逻辑）的落点。
...
```python
{
  ...
  "level": "hard",            # hard 硬约束（缺失阻塞匹配/完成判定）
...
- `level` 默认：固定字段按匹配重要性标注；品类扩展默认 `soft`（可按品类覆盖）。
- `depends_on` 与 `depends_on_value` 组成**联动声明**：`depends_on` 未满足（依赖字段未 SET 或为 WILDCARD/EXCLUDED）时，本字段**不进入 allowed_set、不参与完成判定**。
- 现有 `required`/`optional` 标记**废弃**，统一由 `level` 表达（`required` ≈ `hard`）。
...
### 9.3 确定性算法：fields_for / pending_slots / 动态 Validator

```python
def fields_for(pt: str | None) -> list[dict]:
    """当前品类完整字段（固定 + 品类扩展）；保持 Schema 顺序。"""

def active_fields(state: dict, pt: str | None) -> list[dict]:
    """按依赖声明过滤：depends_on 满足才生效（用于 allowed_set / 判定 / 追问）。"""

def pending_slots(state: dict, pt: str | None) -> list[dict]:
    """当前合法待填集：active_fields 中未填（非 SET）字段；
       排序：level 硬→软→可选；同级别按 Schema 顺序；
       每条带 (field, level, question, options, dep_source)。"""

def next_slot(state: dict, pt: str | None) -> dict | None:
    """本轮追问锚点 = pending_slots 第一项（硬约束优先）。"""

def validate_completion(state: dict, pt: str | None) -> CompletionVerdict:
    """动态 Validator：硬约束（active 且 level=hard）100% SET 且非空 → DONE；
       soft/optional 缺失不阻塞；返回 {done, missing_hard[], missing_soft[]}。"""
```

> **核心立场（红线 5）**：待填槽位的**优先级是规则的输出、模型的输入**——LLM 不决定"接下来问什么"，只负责把该问的说自然。策略层把 `next_slot`（本轮锚点）+ `allowed_set`（合法待填集）作为受控上下文注入推理节点。

**三层受控注入（追问逻辑，回应"依赖式 Schema 下如何传优先级"）**：

| 注入层 | 内容 | 作用 |
| --- | --- | --- |
| 本轮锚点 `next_slot` | 策略层已选定"下一维度"，只注入这一个字段 | 模型首要任务：围绕它自然提问 + 解析回答 |
| 合法待填集 `allowed_set` | 当前可写槽位显式清单（key + 模板 + 级别 + options） | 模型只能在此集合内落值 |
| 动态裁剪的 Tool Schema | `update_requirement_slots` 参数按 `allowed_set` 每轮实时生成 | **结构性强制**：当前不合法字段根本不进 tool 参数 |

- **不跳问**：模型首要任务是当前 `next_slot`；混合意图也只允许顺带写入 `allowed_set` 内值。
- **不问失效字段**：reconcile 清掉失效槽位，`pending_slots` 随状态刷新。
- **问得有理有据**：每条 pending 带 `dep_source`，模型可自然说"因为您选了 X，所以想确认 Y"。
- **兜底**：`reply_text` 空/跑偏 → 退回确定性模板问 `next_slot`；连续无进展触发 `_stall_counter` 熔断。
...
- `update_requirement_slots` 的 parameters 由 `allowed_set` 动态生成：
  ...
  - `extra_constraints` 固定附加：`array of string`（开放扩展）；
  - **`__states__` 固定附加**（三态编码，PoC T3 实证 `{"interfaces":"excluded"}`）：`{"type":"object","description":"三态覆盖：{'<key>':'excluded'} 排除、{'<key>':'wildcard'} 不限；未出现=正常指定 set"}`——排除/不限时槽位值照常写入 + `__states__` 标记。
...
- **现状事实**：`confirm()` 每次生成新 `BuyerRequest`（version 递增）+ 新 `MatchRun`，互不覆盖；`conv.status` 只设过 `confirmed`，从未设过 `closed`；`message()` 不检查会话状态，确认后仍可继续萃取。即"每次提交 = 一份需求档案快照"已成立；坏的是会话级 `status` 承载了提交态。
- **重新设计（收敛）**：
  1. `Conversation.status` 只保留生命周期：`active` / 删除（`deleted_at` 承担删除）；移除 `confirmed`/`closed`。
  2. 提交/匹配状态**全部下沉需求级**：`BuyerRequest`（快照 vN，独立 request_id）+ `MatchRun`（running → ready/failed）各自表达"一次提交"。
...
async def on_confirm(db, conv, user_id, mode: str) -> MatchTrigger:
    # mode: strong（明确授权/强命令） | weak→先 recap 不落快照（SC-22b）
    snapshot = {
        "conversation_id": ...,
        "user_id": ...,
        "version": next_version(...),           # version++
        "structured_demand": to_demand_snapshot(state),  # 三态/排除
        "extra_constraints": state["extra_constraints"],
    }
    # 1) 落库 buyer_requests（快照） 2) 生成需求向量 3) 触发 match_runs（running→ready/failed）
```

- **提交门槛（4.5）**：品类锚点缺失 → 禁止提交；其余硬约束缺失 + 强命令提交 → **允许但强制显著警示缺项**（不静默），对应 SC-25。
...
> 吸收旧第 10 章，按 v2 收敛：A 档并入推理节点（不增加调用），B/C 档规则拦截（零 LLM）。

| 档位 | 触发 | 典型输入 | 处置 | LLM 成本 |
| --- | --- | --- | --- | --- |
| A 领域答疑 | 推理节点判别（intent=qa） | "Linux和Android区别？" | 知识RAG ≤3~5句 + 引导回槽位；不追问下一维度 | 复用推理 1 次 |
| B 厂商相关 | 规则层 `_REDIRECT_PAT` | "这两家谁更好？" | 重定向匹配详情（不回答优劣） | 0 |
| C 无关话题 | 规则层 `_OFFTOPIC_PAT` + 推理节点兜底 | "今天天气" | 守门话术拉回主线 | 0 / 复用推理 |
...
| 合规（越界评价/报价承诺） | 法律/信任风险 | 红线 1/2/3 + B 档规则拦截 + 引导话术 |
...
| 2 LangGraph 状态图 / 意图分类 | 第 8 章（节点与边，route_intent 扩展） |
