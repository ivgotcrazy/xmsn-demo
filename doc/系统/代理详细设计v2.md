...
- **混合意图一次处理**：自由文本（含"问结果 + 填槽"混合）统一进推理节点，LLM 同时产出 `reply_text`（可含引导）与 `tool_call`（槽位更新），不再由规则硬分流；LLM 输出 `intent` 供策略层校验/审计。
- **校验分离**：`tool_call` 参数必须过 Pydantic + 枚举；`reply_text` 仅限长。
- **规则层兜底**：`reply_text` 为空时退回 `decide_question` 确定性追问；引导类用固定模板。
...
| --- | --- | --- | --- | --- |
| ① 消息入口 | 🧩 否 | 接收文本/选项点击 | 全部 | — |
| ② 意图路由 | 🧩 否 | 仅强指令拦截（选项/命令/精确短语/弱收尾/纯引导模板）；自由文本一律进推理节点 | 全部 | 确定性优先；自由文本不硬分流；弱收尾/引导短句加短消息守卫（≤12，见下） |
| ③ 写槽位 | 🧩 否 | 选项确定性直写 | SC-06/13 | 不调 LLM |
| ④ 推理节点 | 🤖 **是** | 上下文包+RAG 一次推断：reply_text + tool_call(slot_delta/extra) + intent；内含 extract_slots / domain_qa / recommend_slots | SC-05~20/28~35 | 混合意图一次处理；Tool Calling |
...
> **② 意图路由实现口径（评审确认 2026-08-13）**：
> - **短消息守卫（N=12）**：弱收尾/引导短语仅在**纯短语短消息**（`len(text) ≤ 12`）时拦截；**带附加内容的混合消息一律放行给推理节点**（防"就这样吧，接口加个USB"被吞掉；SC-22b 走复述引导不直接提交，可恢复）。
> - **模式配置化**：弱收尾/引导短语表做成**配置数据**（settings 或独立 pattern 表），加短语不改代码；精确命令（`/done`、选项点击）仍硬编码常量。
> - **B/C 档双层**：正则"快通道"（零 LLM，命中即 `intent=guide`）+ 推理节点兜底（漏网的自由文本由 LLM 产出 `intent=guide` 再走引导话术）——既便宜又兜得住。
...
#   Intent = OPTION_CLICK | CONFIRM_COMMAND | WEAK_CLOSE | HELP | GUIDE_RESULT | GUIDE_BACK | FREE_TEXT
...
#   ReasoningOutput = { reply_text, tool_call(slot_delta/extra) | None, intent }
