...
> 版本：v2.0 ｜ 定位：**对话 Agent 的完整设计**（效果驱动 + 工程落地） ｜ 状态：设计定稿（大纲《代理设计v2大纲.md》v1.0 评审通过） ｜ 更新：2026-08-12
>
...
| ⑥ 完成判定 | 动态 Validator：硬约束 100% 覆盖（9.3） | SC-21 | 确定性规则 |
...
| ⑧ 确认提交 | 快照+匹配 → closed 语义（提交事件） | SC-22/25 | 单端点 |
...
- `tool_choice`：`{"type": "function", "function": {"name": "update_requirement_slots"}}`（强制走函数调用）。
...
- **代码落点**：`confirm()` 不再写 `conv.status="confirmed"`；`message()` 不按状态门禁（SC-27 分流）。
