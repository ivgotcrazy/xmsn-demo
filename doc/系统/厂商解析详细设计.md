...
> 版本：v1.1 ｜ 状态：指导编码（已对齐 AI核心 D1/D3/D9） ｜ 更新：2026-08-16
>
> 本文档是**厂商能力解析**（输入侧）的详细设计（Low-Level Design），目标**可直接指导编码**：覆盖"资料上传 → 文档解析 → LLM 提取能力档案 → 双轨向量化 → 审核状态"全链路。
>
> **相关文档**：《AI核心总体架构设计v2.md》（D1 同 key / D3 soft 软层 / D9 代表向量）、《供需Schema设计.md》（能力 Schema §4）、《系统架构设计.md》、《核心系统需求.md》、《匹配详细设计.md》（Stage2 消费 structured）、《系统开发计划.md》。
>
> **⚠ 设计变更（2026-08-16，对齐 AI核心 D1/D3/D9，未全文重写）**：
> - **D1 同 key**：现有 `os_support` 等旧 key → **本体同 key**（`os`），消除映射层；`structured_tags` 按本体维度（能力 Schema，供需Schema §4.2）萃取，不再自造 key。
> - **D3 soft 软层**：能力画像 = `{structured: {维度key: {value, confidence, source}}, soft: {tags, summary, doc_chunks}}`；现有 `structured_tags`（structured）+ `summary_text`/`doc_chunks`（soft 部分）对应，另补 **`soft.tags`**（自由能力标签，本体外能力承接，D3）。
> - **D9 代表向量**：REP = `embed(summary_text)`（自然语言全字段，≤400 字）——已同步 §6。
...
class ExtractedCapability(TypedDict):
    ...
    summary_text: str               # 自然语言全字段能力描述 ≤400 字（展示 + REP 向量，D9）
...
# 输出Schema
{
  ...
  "summary_text": "用自然语言将以下全部萃取字段串联成一段能力描述，不超过400字，字段不得遗漏、缺失的不编造：品类/工艺/认证/OS/接口/起订量/交期/月产能/场景/定制/自由标签",
...
- **summary_text**：自然语言串联全部萃取字段的能力描述（含 soft.tags），≤400 字；一物两用：档案展示 + REP 向量（REP=`embed(summary_text)`，D9）。
...
- `summary_text` = 自然语言全字段能力描述（≤400 字，展示 + REP 向量）；
...
| 轨 | 集合 | 内容 | 用途 |
| --- | --- | --- | --- |
| 代表向量 | `vendor_representative` | `summary_text`（自然语言全字段能力描述，1 档案=1 向量，D9） | 匹配通道A |
| 原文块向量 | `doc_chunks` | 文档切块 + `{vendor_id, doc_id, page, chunk_text}` | 引用溯源/查看原文 |

```python
# REP = embed(summary_text)（D9，方案 X）：不再拼 structured_tags
rep_vec = await embed(capability["summary_text"])
```
...
- **质量指标**：解析字段与人工标注一致率（抽样评估）；**字段级 confidence 校准**（低置信度提示补文档）；**summary 字段覆盖性**（必须含全部萃取字段）+ 自然语言可读性；
